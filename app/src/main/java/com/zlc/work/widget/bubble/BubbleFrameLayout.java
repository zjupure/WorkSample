...
public class BubbleFrameLayout extends FrameLayout {
  ...
  public BubbleFrameLayout(Context context, @Nullable AttributeSet attrs) {
    super(context, attrs);
    init(context, attrs);
  }

  private void init(Context context, AttributeSet attrs) {
    TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.BubbleFrameLayout);
    //背景颜色
    int backGroundColor = ta.getColor(R.styleable.BubbleFrameLayout_background_color, Color.WHITE);
    //阴影颜色
    int shadowColor = ta.getColor(R.styleable.BubbleFrameLayout_shadow_color,
        Color.parseColor("#999999"));
    int defShadowSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX,
        4, getResources().getDisplayMetrics());
    //阴影尺寸
    int shadowSize = ta.getDimensionPixelSize(R.styleable.BubbleFrameLayout_shadow_size, defShadowSize);
    mRadius = ta.getDimensionPixelSize(R.styleable.BubbleFrameLayout_radius, 0);
    //三角形方向
    mDirection = ta.getInt(R.styleable.BubbleFrameLayout_direction, BOTTOM);
    mOffset = ta.getDimensionPixelOffset(R.styleable.BubbleFrameLayout_offset, 0);
    ta.recycle();
